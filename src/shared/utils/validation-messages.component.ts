...
export class ValidationMessagesComponent {
    ...
    readonly standartErrorDefs: ErrorDef[] = [
      ...
      {
        error: 'max',
        localizationKey: 'NumberShouldBeLessThanN',
        errorProperty: 'max',
      } as ErrorDef,
        { error: 'email', localizationKey: 'InvalidEmailAddress' } as ErrorDef,
        { error: 'validateTimeGreaterThan', localizationKey: 'End time must be greater than start time.' } as ErrorDef,
        { error: 'pattern', localizationKey: 'InvalidPattern', errorProperty: 'requiredPattern' } as ErrorDef,
        {
            error: 'validateURI',
            localizationKey: 'PleaseEnterValidURL',
        } as ErrorDef,
        {
            error: 'validateNoHtml',
            localizationKey: 'No html is allowed to be entered into the homepage teaser',
        } as ErrorDef,
        { error: 'validateRequired', localizationKey: 'This field is required.' } as ErrorDef,
        {
            error: 'validateMaxLength',
            localizationKey: 'PleaseEnterNoMoreThanNCharacterOnDescription',
            errorProperty: 'data-max-description-length',
        } as ErrorDef,
        {
            error: 'validateMaxLinksLength',
            localizationKey: 'PleaseEnterNoMoreThanNCharacterOnLinks',
            errorProperty: 'data-max-markup-length',
        } as ErrorDef,
    ];
    ...
    getErrorDefinitionMessage(errorDef: ErrorDef): string {
        ...
        if (!errorRequirement) {
            errorRequirement = this.formCtrl?.valueAccessor?.el?.nativeElement?.getAttribute(errorDef.errorProperty);
        }
