...
export class TopBarMenuComponent extends AppComponentBase implements OnInit {
    ...
    getSubItemCssClasses(item, parentItem, depth) {
        let cssClasses = 'sub-menu-link d-inline-block py-3 px-4 mr-6 rounded';

        if (this.isMenuItemIsActive(item)) {
            cssClasses += ' sub-menu-item-active bg-light  mr-6';
        }
    ...
    subMenuClick(item: AppMenuItem, parentItem: AppMenuItem): boolean {

        // School Site Editor sub-menu items need to be identified and tracked separately
        if (parentItem.route === '/app/site-editor/edit-home-page') {
            this._angulartics2.eventTrack.next({
                action: item.name,
                properties: {
                    category: 'Site Editor Click',
                    label: item.route,
                },
            });
        }
